...
namespace GUI
{
    public partial class frm_ShiftControl : Form
    {
        ...
        private void updateMoney()
        {
            ...
            int total = m_500K + m_200K + m_100K + m_50K + m_20K + m_10K + m_5K + m_2K + m_1K;
            if (this.option == 1)
                this.lb_chenh_lech_price.Text = String.Format("{0:#,##}", DTO.DTO_Money.Sales - total);
        ...
        private void btn_submit_Click(object sender, EventArgs e)
        {
            if(option == 0)
            {
                ...
                try
                {
                    ...
                    DTO.DTO_Money.Status1 = true;
                    DTO.DTO_Money.Money = long.Parse(this.lb_total_price_input.Text.ToString().Replace(",","").Trim());
                ...
                long sales = DTO.DTO_Money.Sales;
                long chenhLech = sales - long.Parse(this.lb_chenh_lech_price.Text.ToString().Replace(",", "").Trim());

                try
                {
                    new BUS.BUS_WorkShift().updateWorkShift(frmHethong.workShiftID, float.Parse(DateTime.Now.Hour.ToString() + "." + DateTime.Now.Minute.ToString()));
